    ...
    ArrayList<String> OA = new ArrayList<String>();
        ...
        try {
            ...
            if (tokens == null) {
                ...
                File archivo = new File("Codigo_Optimizado.txt");
            ...
            switch (tokens) {
                        ...
                        //resultado += "IDES" + Error.size() + "                    " + lexer.lexeme + "         " +   "linea " +  Cont.size() + "       " + "Variable no declarada\n";
                            ...
                            if(!ID1.contains(lexer.lexeme)){
                                ...
                                if(ID1.size() == TD1.size()){
                                    String id = ID1.get(ID1.size() -1);
                                    String td = TD1.get(TD1.size() -1);
                    ...
                    case CN:
                        if(TD1.contains("int") && TD1.size() == ID1.size()){
                            Valor.add(lexer.lexeme);
                            String r = Valor.get(Valor.size() -1);
                            //resultado += r + "\n";
                            if(Valor.contains("1")){
                                String t = OA.get(OA.size() -1);
                                if(OA.contains("/") || OA.contains("*")){
                                  //  resultado+= "holanena\n";
                                    int o = OA.size() -1;
                                    OA.set(o, "Nada");
                                }
                    ...
                    case OA:
                        ...
                        break;
                    case SEP:
                        String h = Valor.get(Valor.size() -1);
                        if(h == "Nada"){
                            //resultado+="holabebe";
                        } else {
                            resultado+= ";\n";
